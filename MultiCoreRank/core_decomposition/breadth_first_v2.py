    ...
    while len(vectors_queue) > 0:
        ...
        if number_of_non_zero_indexes == number_of_ancestors:
            ...
            if sum(list(vector)) > level:   # whenever we go down a level in the core lattice
                ...
                # count = count_node_apprence_in_level_above(cores, multilayer_graph)

                #inf_temp = influence

                '''Inf is a dictionary of {node:influence}, influence is the node influence in the core vector
                e.g. 
                ...
                for core_vector, inf in inf_by_core_vector.items():
                    if sum(list(core_vector)) == level:     # calculate the influence of all nodes in current level 
                        for node, _ in inf.items():
                            influence[node] = 0     # reset influence to zero

                for core_vector, inf in inf_by_core_vector.items():
                    if sum(list(core_vector)) == level:     # calculate the influence of all nodes in current level 
                        for node, _ in inf.items():
                            influence[node] += inf_by_core_vector[core_vector][node]
                            # inf_by_core_vector[core_vector][node] = inf_by_core_vector[core_vector][node] * count[node]

                    #del inf_by_core_vector[core_vector]
            ...
            if len(k_core) > 0:
                ...
                '''
                for each node v in in this core vector's core:
                    get current node v influence from influence:
                    get len(k_core)
                    update 
                '''
                ...
                for node in k_core:
                    ancestor_level_node_influence = influence[node]
                    edge_weight = len(k_core)
                        # add influence
                    inf_by_core_vector[vector][node] = ancestor_level_node_influence * edge_weight * level # TODO find number of appearence
...
def print_influence(influence, path):
    ...
    with open(os.path.join(path, "influence_v2_1.txt"), 'w+') as f:
